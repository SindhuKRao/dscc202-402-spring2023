...
filelist = dbutils.fs.ls(BIKE_TRIP_DATA_PATH)
...
events_df.createOrReplaceTempView("my_temp_view")
...
# MAGIC Also the long weekend of President's day on Feb 21st, 2022 explained a dip.
# MAGIC Like this there are dips holidays such as Easter (April 17th), Independence Day (July 4th) etc. which are explaing that less number of people used the bike service.
...
# MAGIC FROM my_temp_view as tab1
# MAGIC INNER JOIN my_weather_view as tab2
# MAGIC ON DATE(tab1.started_at) = DATE(from_unixtime(tab2.dt))
# MAGIC AND HOUR(tab1.started_at) = HOUR(from_unixtime(tab2.dt))
